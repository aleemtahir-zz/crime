		...
		for (int i=0;i<50;i++)
		{
				...
				for(int j=0;j<check.size();j++)
			{
				if ( result.toLowerCase().indexOf(check.get(j).toString().toLowerCase()) != -1 )
				{
					...
					Resource loc=model.createResource(uri+location);
					...
					Property kill=model.getProperty(uri+"Killed");
					Property kid=model.getProperty(uri+"Kidnap");
					Property inj=model.getProperty(uri+"Injured");
					Property loca=model.getProperty(uri+"hasLocation");
					...
					NewsIns.addLiteral(arr, arrested);
					NewsIns.addLiteral(inj, injured);
					NewsIns.addLiteral(kill, killed);
					NewsIns.addLiteral(kid, kidnap);
					NewsIns.addLiteral(da, news.get(i+50).toString());
					model.add(NewsIns,prOccur,DateIns);
					model.add(NewsIns,loca,loc);
					n++;
					flag++;
					vict=0;
					killed=0;
